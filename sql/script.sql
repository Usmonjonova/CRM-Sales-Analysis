...
-- avg deal stage date 
select avg(close_date - angage_date )
...
-- Total opportunity count by account
SELECT account,
...
select
    account,
    count(opportunity_id) as total_orders
from sales_pipeline
group by account
...
-- product demand (most sold products)
select
    product,
    count(opportunity_id) as total_units
from sales_pipeline
group by product
...
select st.regional_office, sum(sp.close_value) as total_revenue from sales_teams st
...
with state_sales as (
    select office_location, avg(close_value) as avg_price from accounts a
...
with customer_sales as (
    select account, avg(revenue) as avg_revenue from accounts
    group by account
)
